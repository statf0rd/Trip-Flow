...
fun MapPickerView(
    ...
    DisposableEffect(mapView) {
        val listener = object : CameraListener {
            override fun onCameraPositionChanged(
                ...
                if (finished) {
                    onLocationPicked(position.target.latitude, position.target.longitude)
                }
            }
        }
    ...
    AndroidView(
        ...
        update = { view ->
            val mapObjects = view.map.mapObjects
            mapObjects.clear()
            val center = view.map.cameraPosition.target
            mapObjects.addPlacemark(
                center,
                iconProvider(iconCache, 0xFFFF6B5B.toInt(), "\u2022"),
                markerStyle(scale = 1.2f)
            ).apply { zIndex = 10f }
        }
    )
}
